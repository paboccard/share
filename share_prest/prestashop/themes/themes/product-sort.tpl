...
<form id="productsSortForm{if isset($paginationId)}_{$paginationId}{/if}" action="{$request|escape:'html':'UTF-8'}" class="productsSortForm">
	<div class="select selector1">
		...
		<select id="selectProductSort{if isset($paginationId)}_{$paginationId}{/if}" class="selectProductSort form-control">
			<option value="{if $page_name != 'best-sales'}{$orderbydefault|escape:'html':'UTF-8'}:{$orderwaydefault|escape:'html':'UTF-8'}{/if}"{if !in_array($orderby, array('don', 'price', 'name', 'quantity', 'reference')) && $orderby eq $orderbydefault} selected="selected"{/if}>--</option>
			<option value="don:asc"{if $orderby eq 'don' AND $orderway eq 'asc'} selected="selected"{/if}>Don croissant</option>
			<option value="don:desc"{if $orderby eq 'don' AND $orderway eq 'desc'} selected="selected"{/if}>Don décroissant</option>
			{if !$PS_CATALOG_MODE}
